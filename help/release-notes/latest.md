...
| ----------- | ---------- | ---------- |
| **13-month expiration of saved cust_visids**  | March 20, 2024 | The May 2024 release of the Analytics Hit processing engine will start enforcing a 13-month expiration of saved cust_visids. If the report suite has "Enable Visitor Stitching" enabled, this setting is used for finding the cust_visid for a visid_high/visid_low value with no cust_visid on the hit. The "Enable Visitor Stitching" feature looks for a cust_visid previously used for a visid_high/visid_low if a hit for the visid_high/visid_low has no cust_visid on the hit.  Currently, there is no expiration of the mapping of a cust_visid for a visid_high/visid_low. With this release, if it has been 13 months or more since the visid_high/visid_low has had a cust_visid on a hit, the mapping will expire. | 
