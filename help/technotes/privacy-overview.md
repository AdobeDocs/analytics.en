...
| Anonymized visitor ID's | Data collection libraries generate and reference a visitor ID for each browser visiting your site. This ID is stored in a cookie. If a data collection library cannot set a cookie identifier, the library uses a fallback method of anonymous visitor identification. This method involves tying related hits to the same visit using IP address and user agent string. If your organization has IP obfuscation enabled, this setting is honored. See [Adobe Analytics and browser cookies](cookies/cookies.md) for more information. | [Unique visitors](../components/metrics/unique-visitors.md) |
...
Adobe maintains three data processing locations for Adobe Analytics. These sites receive raw data and process it into a report suite, which is optimized for data storage and reporting retrieval. These data processing locations reside in the United States (Oregon), United Kingdom (London), and Singapore. See [Adobe Analytics security overview](https://www.adobe.com/content/dam/cc/en/trust-center/ungated/whitepapers/experience-cloud/adb-analytics-security-wp.pdf){target=_blank} for more information.
