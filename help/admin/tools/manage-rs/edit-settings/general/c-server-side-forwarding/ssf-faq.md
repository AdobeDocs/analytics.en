...
|Q: What if I have multi-suite tagged report suites that are mapped to separate CX Enterprise Orgs?|You should never send data from a single Analytics hit to two report suites that belong to separate CX Enterprise Orgs, but if this does occur, we will only forward the hit to the CX Enterprise Org matching the Identity Service setup on the page.|
|Q: What if I have multi-suite tagging and only one of my report suites is mapped to my CX Enterprise Org and the other is not?|We will forward the hit to the corresponding data collection server for the CX Enterprise Org on your mapped report suite, however since the non-mapped report suite will not have an associated data source in Audience Manager, no data will be recorded for the un-mapped report suite in Audience Manager.|
|Q: What if I have a report suite that is mapped to multiple CX Enterprise Orgs?|Analytics will consider this report suite as unmapped and will not allow server side forwarding to be enabled for this report suite. Contact customer care to resolve this mapping issue.|
...
|Q: What if we have two CX Enterprise Orgs (or Adobe Audience Manager instances) and want to share data between both CX Enterprise Orgs? Can I server side forward a single Analytics hit to multiple CX Enterprise Orgs?|No. If you need to share data collected under one CX Enterprise Org to another CX Enterprise Org, we recommend sending any applicable audiences from one Audience Manager instance to another using the audience marketplace.|
...
>[!MORELIKETHIS]
>
>* [Server-Side Forwarding](/help/admin/tools/manage-rs/edit-settings/general/c-server-side-forwarding/ssf.md)
