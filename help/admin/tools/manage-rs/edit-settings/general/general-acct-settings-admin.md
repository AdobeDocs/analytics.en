...
| Time Zone | Determines the date and time associated with your report data. If the selected time zone observes daylight saving time, hourly data is automatically adjusted to reflect that. Changing the time zone for a live report suite creates either a spike or gap in report data. Adobe recommends making this change during non-peak hours to minimize impact. |
