...
* Cross-Device Analytics uses a virtual report suite and report time processing, which have their own limitations, like no current support for Marketing Channels Variables. See [Virtual report suites](../vrs/vrs-about.html) and [Report time processing](../vrs/vrs-report-time-processing.html) for more information on these limitations.
