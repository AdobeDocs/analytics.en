...
## Changes in labelling and definition

Below is a list of specific issues with how operating system has been represented in the User Agent and so in Analytics reporting. 
...
As of January 2023, the User Agent in all browsers show the Mac OS version as 10.15.7 even for newer versions. This was done because including version 11 in the UA apparently caused problems with some websites. Apple also notes that including an incorrect OS version in the UA provides some privacy benefits. 

Note that client hints include the correct version in the platform version hint ("Sec-CH-UA-Platform-Version"). This is a high entropy hint so is not automatically collected by Adobe. See the Adobe Analytics Hints FAQ for detail on how to collect high entropy hints.
