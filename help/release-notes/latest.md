...
| --- | --- | --- |
| **EOL of [!UICONTROL People] metric** | February 28, 2023 | With the deprecation of the [[!UICONTROL Device Co-op]](https://experienceleague.adobe.com/docs/discontinued/using/device-co-op.html), the [[!UICONTROL People]](/help/components/metrics/people.md) metric is no longer relevant. In the near future (date TBD), we will remove the [!UICONTROL People] metric. At that point, we will redirect its data to the [!UICONTROL Unique Visitor] metric to prevent projects, segments and calculated metrics from breaking. |
