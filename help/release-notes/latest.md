...
| **Classification sets - consolidation** | Combine classifications from different report suites into a consolidated data set. The consolidated data set can be used in Classification sets or as a lookup data set in CJA. |  | June 7, 2023 |
| **Classification sets - rule builder** | Use the classification rule builder in the current classification set architecture. |  | June 7, 2023 |
| **Classification sets - automated import** | You can now automatically import classification set data from cloud storage destinations. |  | June 7, 2023 |
| **New AppMeasurement variable** | The variable `doubleEncodeLinkParameters` accommodates edge cases where implementations encode multi-byte characters in link tracking variables. Most implementations do not need to define this variable. |  | June 7, 2023 |
