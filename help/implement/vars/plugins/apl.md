...
# Adobe Plugin: apl (appendToList)

## Plugin Purpose

### What does this plugin do?

The apl (appendToList) plugin allows you to "safely" add new values to list-delimited variables (e.g. events, linkTrackVars, listVars, etc.).
...
Using the apl plugin will prevent potential duplicate values from being added to a list.
For example, assume...

```javascript
s.events = "event1,event2";
```
If you use apl to try and add "event1" to s.events, the logic would not add it since s.events already contains "event1".
...
* For any Launch Rule that you want to use the plugin in, add an [!UICONTROL action] with the following configuration:
	...
	* Action Type: Initialize apl
* Save and publish the changes to the rule

## How to Run the Plugin

When calling the apl plugin (via JavaScript), be sure to pass in the following arguments:
...
* Removed console warnings to reduce size - use documentation for help with troubleshooting
* Added inList 2.1
### 3.1 (2018-04-22)
