...
In September 2019 Adobe Analytics is introducing early access for Analytics Ultimate customers to a powerful new feature called Journey IQ: Cross-Device Analytics. Cross-Device Analytics is a feature that transforms Analytics from a device-centric view to a person-centric view. This feature uses the Adobe Experience Platform Identity Service Co-op Graph or Private Graph to identify which devices belong to individuals, and stitch them together. As a result, analysts can understand user behavior that crosses browsers, devices, or apps. Using CDA you can answer questions such as:
...
When devices are stitched, variable persistence is carried over across devices. For example, suppose a user first visits your site through an advertisement on their desktop computer. That user finds your mobile app, installs it, and eventually makes a purchase on their mobile device. With Cross-Device Analytics, revenue can be attributed to the ad they clicked on their desktop computer.
...
As of September 2019, Cross-Device Analytics requires the following. Work with teams within your organization and your Adobe Account Manager to ensure you meet all of the following.

> [!IMPORTANT] Failure to meet all prerequisites may result in the inability to enable Cross-Device Analytics, or may result in poor results when stitching data.
...
* Contact your organization's Account Manager to establish these key points:
  ...
  * Your organization must agree to allow Adobe to process and store Analytics data on Microsoft Azure servers. This is where the graph lives and where stitching is performed. As such, Adobe Analytics data will be passed back-and-forth between Adobe's data processing center and Adobe's presence in Microsoft Azure.
...
* Legacy Analytics ID's are not supported. Only visitors with Experience Cloud ID's will be stitched.
...
* Cross-Device Analytics is presented in a virtual report suites which uses report time processing, and therefore inherits all limitations of these capabilities. See [Virtual report suites](../vrs/vrs-about.md) and (link) for more information.
* New devices visiting your site can take up to two weeks to be processed by the Co-op Graph or Private graph. This means the level of stitching in CDA for the most recent two weeks will be lower than for date ranges older than two weeks. Adobe plans to improve the Adobe Experience Platform Identity Service to stitch new devices real-time in the future.
