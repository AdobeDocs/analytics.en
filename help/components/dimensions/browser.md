...
Some dimension items contain `.999` as the minor version number, such as `"Chrome 148.999"`. This value indicates that Adobe could not reliably determine the browser's minor version. When Chrome or Edge browsers send requests without [client hints](/help/technotes/client-hints.md), the minor version in the user agent string is not considered trustworthy. Instead of inflating dimension items with potentially inaccurate minor versions, Adobe replaces those minor versions with `.999`. Similarly, if any browser reports an unusually high version number (above 99999), Adobe normalizes it to `999.999`.
