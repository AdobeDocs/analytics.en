...
This dimension collects data from the [`pev2` query string](/help/implement/validate/query-parameters.md) in image requests for hits that also have the `pe` query string with the value of `lnk_d`. If `pev2` is not provided, the link URL (`pev1`) is used as the dimension value instead. Hits where `pe` equals `lnk_e` or `lnk_o` populate the [Exit link](exit-link.md) or [Custom link](custom-link.md) dimensions instead. When a link name is explicitly provided, the maximum length is 100 bytes; values derived from the link URL are not subject to this limit.
