...
1. Select either a predefined template or an existing report suite to use as a [template](/help/admin/tools/manage-rs/rs-templates/report-suite-templates.md).

   >[!NOTE]
   >
   >Only settings can be copied, not the data. If Customer Care is copying the settings over, you will need to provide a written confirmation to the disclaimer provided by Customer care about the risks involved. See [Settings not copied from a source report suite](/help/admin/tools/manage-rs/new-rs/settings-not-copied-from-rs.md) for more information.

1. Fill in the fields described in [New Report Suite](/help/admin/tools/manage-rs/new-rs/new-report-suite.md).
...
**After logging in to CX Enterprise, the Analytics icon is greyed out.**
