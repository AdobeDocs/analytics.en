...
Most data object fields in this table coincide with a [mapped XDM field](xdm-var-mapping.md). If you set both a given data object field and its respective XDM field, the data object field takes priority. For example, if the field `data.__adobe.analytics.events` is present, it overwrites all event-related XDM object fields. During Adobe Analytics ingestion, values mapped from XDM are applied first, then recognized data object fields are applied and can overwrite previously set values when they map to the same Analytics variable. If you set a data object field to an empty string (`""`), the Analytics variable that it maps to is set to a blank value for the hit, even if a corresponding XDM field contained a value.
